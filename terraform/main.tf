...
resource "azurerm_network_security_group" "nsg" {
  ...
  # App port is open only inside the VNet by default (monitor VM can reach it).
  security_rule {
    name                       = "allow-app-vnet"
    priority                   = 110
    direction                  = "Inbound"
...
resource "azurerm_public_ip" "mon_pip" {
...
resource "azurerm_network_interface" "mon_nic" {
  ...
  ip_configuration {
    ...
    subnet_id                     = azurerm_subnet.subnet.id
    private_ip_address_allocation = "Dynamic"
    public_ip_address_id          = azurerm_public_ip.mon_pip.id
  }
}
...
resource "azurerm_network_interface_security_group_association" "mon_assoc" {
